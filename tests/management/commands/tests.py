...
class Command(NoArgsCommand):
    def handle(self, **options):
        ...
        #~ 
        #~ for act_ids in ActIds.objects.filter(src="index", act__validated=2, no_unique_type="COD", act__nb_lectures=2):
            #~ act=act_ids.act
            #~ 
        ...
        #~ writer.writerow(headers)
        #~ 
        #~ for act_ids in ActIds.objects.filter(src="index", act__validated=2).exclude(no_unique_type__in=["COD", "SYN"]):
            #~ act=act_ids.act
            #~ 
