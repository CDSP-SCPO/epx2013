...
def q96():
    ...
    for key, value in variables.iteritems():
        ...
        print question
        res=init_cs()
        res=get_by_cs(res, variable=variable, filter_vars={"validated": 2, variable+"_isnull": False, **value})
        write_cs(question, res, percent=1)

        question="Durée DureeTotaleDepuisTransCons moyenne"+key+" par année"
        print question
        res=init_year()
        res=get_by_year(res, variable=variable, filter_vars={"validated": 2, variable+"_isnull": False, **value})
        write_year(question, res, percent=1)

        question="Durée DureeTotaleDepuisTransCons moyenne"+key+" par année et par secteur"
        print question
        res=init_cs_year()
        res=get_by_cs_year(res, variable=variable, filter_vars={"validated": 2, variable+"_isnull": False, **value})
