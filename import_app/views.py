...
from import_app.models import ImportAdoptPC, ImportDosId, ImportNP
...
def save_2_tables(row, fields, fields_fk, party_family=""):
	...
	exist: True if the instance already exists (fields_to_get and fields_to_get), False otherwise [boolean]
	"""
...
def get_data_dos_id(row):
	"""
	FUNCTION
	get a string (row from csv file) and put its content into an instance of ActIds
	PARAMETERS
	row: row from the csv file [row object]
	RETURN
	instance: instance of the model with the extracted data [ActIds model instance]
	msg: id of the row, used to display an error message [string]
...
def get_data_act(row):
	...
	try:
		...
		#extra field not to use for the get but for the create part of get_or_create
		defaults={}
...
def get_data_adopt_pc(row):
	...
	instance: instance of the model with the extracted data [Act model instance]
	msg: id of the row, used to display an error message [string]
	exist (not created): True if the instance already exists (fields_to_get and fields_to_get), False otherwise [boolean]
	"""
	ids_row={}
	ids_row["releve_annee"]=int(row[0])
	ids_row["releve_mois"]=int(row[1])
...
def get_data_np(row):
	"""
	FUNCTION
	get a string (row from csv file) and put its content into an instance of NP
	PARAMETERS
	row: row from the csv file [row object]
	RETURN
	instance: instance of the model with the extracted data [NP model instance]
	msg: id of the row, used to display an error message [string]
	exist (not created): True if the instance already exists (fields_to_get and fields_to_get), False otherwise [boolean]
	"""
	ids_row={}
...
def import_table(csv_file, import_type):
	"""
	FUNCTION
	open a csv file and save its variables in the database (in one table -> used for dos_id, act, adopt_pc, gvt_compo and np)
	PARAMETERS
...
def import_view(request):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			#importation of dos_id, act, adopt_pc, gvt_compo or np file
			if file_to_import in ["dos_id","act","adopt_pc","gvt_compo", "np"]:
